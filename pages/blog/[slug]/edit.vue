          ...
          <div class="md:grid grid-cols-2 gap-4">
            ...
            <div class="mb-4">
              ...
              <div class="mb-4">
                ...
                <textarea
                  class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                  id="content" rows="12" placeholder="Enter content"
                  v-model="content"
                ></textarea>
              </div>
            </div>
...
onMounted(async () => {
  const article = await $fetch(`${apiBase}/api/blog/${route.params.slug}`, {
    headers: token.value ? { Authorization: `Bearer ${token.value}` } : {},
  })
  articleId.value = article._id
  title.value = article.title
  description.value = article.description
  view.value = apiBase + article.img
...
async function handleRemove() {
  const response = await $fetch(`${apiBase}/api/blog/${route.params.slug}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${token.value}` },
  })
  if (response.success) navigateTo('/blog')
