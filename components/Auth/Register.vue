<template>
  <div>
    <div class="bg-red-100 border-t border-b border-red-500 text-red-700 px-4 py-3 mb-4" role="alert" v-if="error.request">
      <p class="font-bold">Registration was failed!</p>
    ...
    <div class="flex flex-wrap mb-3">
      <div class="w-full text-left">
        <label class="block tracking-wide text-gray-700 text-xs font-bold mb-2">Confirm Password</label>
        <input type="password" placeholder="Enter your password" v-model="confirmPassword" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" />
        <div class="text-red-500 text-sm font-medium">{{ error.confirmPassword }}</div>
      </div>
    </div>
    ...
    <div class="text-center mt-4">
      ...
      <span class="text-blue-700 hover:text-blue-900 ml-2 cursor-pointer" @click="$emit('login')">Sign in</span>
    </div>
  </div>
</template>
...
async function register() {
  ...
  if (password.value !== confirmPassword.value) {
    error.value = { ...error.value, confirmPassword: "Password doesn't match" }
    return
  }
  try {
