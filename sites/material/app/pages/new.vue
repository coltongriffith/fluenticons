<template>
  <div class="container mx-auto px-4 sm:px-8 py-8">
    ...
    <h1 class="text-3xl sm:text-4xl font-bold mb-4">New Material icons</h1>
    <div class="prose dark:prose-invert max-w-3xl mb-8">
      <p>
        Google adds new symbols to Material Symbols regularly. This page lists the icons added to
        Materialicons in each update, newest first — the site checks for a new release every
        week. Follow along with the
        <a href="/feed.xml">RSS feed</a>, or <NuxtLink to="/browse/">browse every icon A–Z</NuxtLink>.
      </p>
    </div>
    <p v-if="!updates.length" class="text-gray-600 dark:text-gray-300">
      No new icons yet — this page lists them after Google's next release.
    </p>
...
// Until an update adds icons, the page only says so (and stays out of search).
const latest = updates.value[0];
const description = latest
  ? `The newest Google Material Symbols: ${latest.icons.length} added ${latest.label}. Free SVG, PNG and code for HTML, React, Flutter and more.`
  : "New Google Material Symbols appear here as they're added to Materialicons.";
useSeo({ title: "New Material icons", description, path: "/new", noindex: !latest });
if (latest) {
  useAdsense();
  useCollectionJsonLd({
    name: "New Material icons",
    description,
    path: "/new",
